...
class MyScrollSnapList extends StatefulWidget {
  ///List background
  ...
  ///Method to manually trigger focus to an item. Call with help of `GlobalKey<MyScrollSnapListState>`.
  ...
  ///
  ///Input parameter is distance between item position and center of MyScrollSnapList (Negative for left side, positive for right side)
  ///
  ...
  ///(e.g scroll items vertically if `MyScrollSnapList` axis is `Axis.horizontal`)
  ...
  @override
  MyScrollSnapListState createState() => MyScrollSnapListState();
}

class MyScrollSnapListState extends State<MyScrollSnapList> {
  ...
  double calculateScale(int index) {
    //scroll-pixel position for index to be at the center of MyScrollSnapList
  ...
  double _calcCardLocation(
    ...
    //trigger onItemFocus
    if (cardIndex != _previousIndex) {
      _previousIndex = cardIndex;
  ...
  double calculateListPadding(BoxConstraints constraints) {
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (ctx, constraints) {
          final listPadding = calculateListPadding(constraints);
          return GestureDetector(
